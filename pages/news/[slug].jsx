...
export default function Article({ posts, otherPosts, publishedDate }) {
  const currentExcluded = _.shuffle(otherPosts).filter(
    (item) => item.id != posts.id
  );
  const shuffleArray = currentExcluded
...
export async function getStaticProps({ params }) {
  ...
  const publishedDay = moment(posts.created_at).format("dddd");
